...
export default function EditorTool() {
  ...
  return <section className="editor-app">
    <div className="editor-shell">
      ...
      <aside className="editor-summary"><span>미션 맵 초안</span><strong>{questionCount}문제 · {promptModes.find((mode) => mode.id === promptMode)?.title} 출제 · {solveModes.find((mode) => mode.id === solveMode)?.title} 풀이</strong><p>현재는 브라우저 안에서만 동작하는 데모입니다. 다음 단계에서 저장/불러오기와 실제 이미지 타깃 생성으로 확장할 수 있습니다.</p></aside>
    </div>
  </section>;
}
